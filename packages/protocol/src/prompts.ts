...
export function buildEvaluationQuestions(shapes: { id: string; label: string; props?: unknown; type?: string }[]) {
  const rule = `${CORE_INSTRUCTIONS}\nEvaluate whether the latest cause completes a useful opportunity in the surrounding exchange. Prefer no intervention when a human answered, a question is for a named human, the topic moved on, or the contribution duplicates recent work or an open/dismissed suggestion without new evidence. Tentative proposals and silence are not agreement. `;
  return {
    addressed: { type: "boolean" as const, instructions: rule + "Is the latest cause an actionable request directed at the canvas assistant, rather than conversation between humans?", criteria: { true: "A direct request to the assistant to answer, retrieve data, or change the canvas.", false: "Human-to-human conversation, quoted requests, vague observations, or no assistant request." } },
    worthCapturing: { type: "boolean" as const, instructions: rule + "Would a brief grounded contribution help: answering an open question from supplied context, surfacing evidence, checking a conflict, or drafting a concrete update? Offer substantial work only with available information. Do not intervene merely because someone spoke or moved a shape." },
    intent: { type: "choice" as const, instructions: rule + "Choose the primary action appropriate to the latest cause.", criteria: { answer: "Answer from supplied evidence or clarify an explicit request.", capture: "Draft a small item for a concrete idea, confirmed decision, or unresolved question.", update: "Draft a targeted correction using new agreed information.", lookup: "Offer investigation using available sources; await approval.", evidence: "Surface evidence supporting or challenging a claim with scope and uncertainty.", align: "Ask about an earlier agreement conflicting with a new constraint; do not assume consensus.", none: "No useful grounded contribution, or already handled." } },
    relatedShape: { type: "choice" as const, instructions: rule + "Select the existing shape most directly referenced, or none if ambiguous or unrelated.", criteria: Object.fromEntries([["none", "No clearly related shape"], ...shapes.map((shape, index) => [`shape_${index}`, JSON.stringify(shape)])]) },
    needsExternalData: { type: "boolean" as const, instructions: rule + "Would fulfilling the cause require data not already in the supplied thread or canvas?" },
    captureWish: { type: "score" as const, instructions: rule + "Rate the value of a grounded contribution or small draft. Penalize duplicates, weak evidence, speculative captures, and interruptions.", criteria: ["No value, unsupported or duplicate", "Weak, speculative or interrupting", "Concrete grounded help but optional", "Clearly useful evidence, answer or draft", "Important alignment conflict or essential grounded answer"] },
  };
}
