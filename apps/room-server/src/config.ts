...
import { NebiusClassifier } from "./nebius-classifier";
...
export function configFromEnv(env: NodeJS.ProcessEnv = process.env): ServerConfig {
  ...
  if (mode !== "disabled" && mode !== "jev" && mode !== "nebius") throw new Error("invalid KAN_CLASSIFIER");
  if (mode === "jev" && !env.AI_GATEWAY_API_KEY) throw new Error("jev requires AI_GATEWAY_API_KEY");
  if (mode === "nebius" && !env.NEBIUS_API_KEY?.trim()) throw new Error("nebius requires NEBIUS_API_KEY");
  ...
  const classifier = mode === "disabled" ? null : mode === "nebius"
    ? new NebiusClassifier(env.NEBIUS_API_KEY!, env.NEBIUS_CLASSIFIER_MODEL)
    : new JevClassifier();
  const video =
