...
test("only a start-of-message invocation creates an explicit act trigger", async () => {
...
test("transcript hey kan invokes while imported history remains inert", async () => {
  const ctx = await setup({ classifier: null });
  try {
    const user = await registerUser(ctx.base);
