...
test("migration 2 backfills legacy event payload and asset membership junction", () => {
  ...
  try {
    ...
    db.exec("DROP TABLE asset_rooms; DROP TABLE pending_classification; ALTER TABLE events DROP COLUMN data; UPDATE meta SET v='1' WHERE k='schema_version'");
    db.close(); db = openDb(path);
    assert.equal(db.prepare("SELECT v FROM meta WHERE k='schema_version'").get()!.v, "2");
