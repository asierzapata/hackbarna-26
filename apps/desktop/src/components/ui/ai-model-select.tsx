...
export interface ModelSelectorTriggerProps extends Omit<HTMLMotionProps<"button">, "children"> {
  children?: React.ReactNode
}

export type ModelSelectorValueProps = React.HTMLAttributes<HTMLSpanElement>

export interface ModelSelectorContentProps extends Omit<HTMLMotionProps<"div">, "children"> {
  children?: React.ReactNode
  side?: "top" | "bottom"
}
...
function ModelLabel({ model, selection }: { model?: AiModel; selection: AiModelSelection }) {
  if (!model) return <span className="text-muted-foreground">Select model</span>
  const modifiers = [
...
export function ModelSelector({
  ...
  const selectModel = React.useCallback((id: string) => {
    ...
    setSelection(resolveSelection(models, configCache.current[id] ?? defaultSelectionFor(model)))
    setOpen(false)
    triggerRef.current?.focus()
  }, [models, setOpen, setSelection])

  const patchSelection = React.useCallback((patch: Partial<AiModelSelection>) => {
    setSelection((previous) => {
      const next = resolveSelection(models, { ...previous, ...patch, id: patch.id ?? previous.id })
  ...
  return (
    ...
    >
      <div data-slot="model-selector" className={cn("relative inline-flex", className)}>
        {children}
      </div>
    </SelectorContext.Provider>
  )
...
export const ModelSelectorTrigger = React.forwardRef<HTMLButtonElement, ModelSelectorTriggerProps>(
  ({ children, className, onClick, disabled, ...props }, ref) => {
    const { open, setOpen, triggerRef, contentId, selectedModel, disabled: rootDisabled } =
...
export const ModelSelectorContent = React.forwardRef<HTMLDivElement, ModelSelectorContentProps>(
      ...
      <AnimatePresence>
        {open ? (
          ...
          >
            ...
            {editingId && selectedModel?.id === editingId ? <ModelSettings model={selectedModel} /> : null}
          </motion.div>
        ) : null}
      </AnimatePresence>,
      document.body
...
function ModelSettings({ model }: { model: AiModel }) {
  ...
  const options = (model.efforts ?? []).map((effort) => ({ label: EFFORT_LABEL[effort], patch: { effort } }))
  const contexts = (model.contexts ?? []).map((context) => ({ label: formatContext(context) ?? String(context), patch: { context: formatContext(context) ?? String(context) } }))
  return (
    <aside className="flex w-48 shrink-0 flex-col gap-2 rounded-xl border border-border p-2">
      ...
      {options.length ? <SettingGroup label="Effort" options={options} selected={selection.effort} onSelect={(effort) => patchSelection({ id: model.id, effort: effort as AiModelEffort })} /> : null}
      {contexts.length ? <SettingGroup label="Context" options={contexts} selected={selection.context} onSelect={(context) => patchSelection({ id: model.id, context })} /> : null}
      {model.supportsFast || model.supportsThinking ? <div className="flex flex-col gap-0.5"><span className="px-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Modes</span>{model.supportsFast ? <SettingButton label="Fast" selected={!!selection.fast} onClick={() => patchSelection({ id: model.id, fast: !selection.fast })} /> : null}{model.supportsThinking ? <SettingButton label="Thinking" selected={!!selection.thinking} onClick={() => patchSelection({ id: model.id, thinking: !selection.thinking })} /> : null}</div> : null}
    </aside>
  )
}

function SettingGroup({ label, options, selected, onSelect }: { label: string; options: Array<{ label: string; patch: Record<string, string> }>; selected?: string; onSelect: (value: string) => void }) {
  return <div className="flex flex-col gap-0.5"><span className="px-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{label}</span>{options.map((option) => <SettingButton key={option.label} label={option.label} selected={selected === option.label} onClick={() => onSelect(option.patch[Object.keys(option.patch)[0]])} />)}</div>
}

function SettingButton({ label, selected, onClick }: { label: string; selected: boolean; onClick: () => void }) {
  return <button type="button" aria-pressed={selected} onClick={onClick} className={cn("flex items-center justify-between rounded-lg px-2 py-1.5 text-left text-xs", selected ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted/70 hover:text-foreground")}><span>{label}</span>{selected ? <CheckIcon className="size-3.5" aria-hidden /> : null}</button>
}

export function ModelSelectorKit({ models = DEFAULT_AI_MODELS, value, defaultValue, onValueChange, disabled, className, "aria-label": ariaLabel }: ModelSelectorKitProps) {
  return <ModelSelector models={models} value={value} defaultValue={defaultValue} onValueChange={onValueChange} disabled={disabled} className={className} aria-label={ariaLabel}><ModelSelectorTrigger><ModelSelectorValue /></ModelSelectorTrigger><ModelSelectorContent /></ModelSelector>
}
