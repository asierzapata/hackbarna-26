...
export function buildCanvasPrompt(text: string, transcript: ConversationLine[] = [], shapeIds: string[] = []) {
  return [
    ...
    "Use the kan-canvas MCP tools to perform requested canvas changes, not just describe them. Start with getCanvas. You have addNode, updateNode, removeNodes, connectNodes, arrange, focusNodes and getCanvas. No filesystem, terminal or other tools are permitted.",
