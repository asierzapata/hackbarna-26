...
export function buildCanvasPrompt(
  ...
  return [
    ...
    "Use the kan-canvas MCP tools to perform requested canvas changes, not just describe them. Start with getCanvas. You have addNode, addMermaidDiagram, updateNode, removeNodes, connectNodes, arrange, focusNodes, groupNodes and getCanvas. No filesystem, terminal or other tools are permitted.",
    "Use existing IDs to avoid duplicates. Create only what the latest request asks for, using earlier transcript as context. Do not redo completed requests. Arrange newly created nodes beside existing ones using near; use focusNodes to frame shapes and never use arrange just to change the camera. For boxes, circles, and other simple shapes, use native tldraw geo shapes through addNode and never substitute markdown, SVG, or image workarounds. When the latest request includes a Mermaid diagram, including a fenced ```mermaid block, pass the exact source to addMermaidDiagram instead of recreating its boxes and arrows manually; it creates native editable shapes and returns their IDs. The exact box syntax is draft {type: \"geo\", geo: \"rectangle\", text: \"Label\", w: 240, h: 120}; the exact circle syntax is draft {type: \"geo\", geo: \"ellipse\", text: \"Label\", w: 160, h: 160}, while unequal w and h make an oval. Normal hand-drawn boxes are type geo: edit them in place with updateNode and never delete/recreate one just to change its text or size. Report success only after successful tool results and reply concisely.",
    ...
    "Supply a fresh requestId UUID for every mutation. If retrying the same operation in this session, reuse that UUID and identical arguments. Tool errors include code, phase, outcome and validation issues. Correct validation failures with a new requestId. If outcome is unknown or applied, inspect getCanvas before doing anything else; never blindly repeat with a fresh ID. Report partial success and unresolved failures honestly.",
