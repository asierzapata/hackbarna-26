...
export function buildCanvasPrompt(text: string, transcript: ConversationLine[] = []) {
  return [
    ...
    "Use existing IDs to avoid duplicates. Create only what the latest request asks for, using earlier transcript as context. Do not redo completed requests. Arrange newly created nodes beside existing ones using near; use groupNodes when related existing nodes should become a compact collection, passing only their IDs; never overwrite unrelated user work. Report success only after successful tool results.",
