...
fn read_message(reader: &mut impl BufRead) -> Result<Option<Value>, String> {
    ...
    let size = reader.take((MAX_MESSAGE + 1) as u64).read_line(&mut line).map_err(|_| "Could not read bridge message")?;
    if size == 0 { return Ok(None); }
    if size > MAX_MESSAGE || !line.ends_with('\n') { return Err("Bridge message too large or incomplete".into()); }
    serde_json::from_str(&line).map(Some).map_err(|_| "Invalid bridge JSON".into())
}

fn write_message(writer: &mut impl Write, value: &Value) -> Result<(), String> {
    let text = value.to_string();
    if text.len() > MAX_MESSAGE { return Err("Bridge response too large".into()); }
    writeln!(writer, "{text}").and_then(|_| writer.flush()).map_err(|_| "Could not write bridge message".into())
}

impl CanvasMcp {
    pub fn start(app: AppHandle, tools: Vec<Value>) -> Result<Arc<Self>, String> {
        Self::with_emitter(tools, Box::new(move |event| {
            app.emit_to("main", "canvas:tool", event).map_err(|error| error.to_string())
        }))
    }
    ...
    fn with_emitter(tools: Vec<Value>, emit: Box<Emit>) -> Result<Arc<Self>, String> {
        if tools.len() != TOOL_NAMES.len() || TOOL_NAMES.iter().any(|name| {
            ...
            return Err("Expected the nine canvas tool schemas".into());
        }
        ...
        std::thread::spawn(move || {
            ...
            while weak.strong_count() > 0 {
                match listener.accept() {
                    Ok((mut socket, _)) => {
                        if connections.load(std::sync::atomic::Ordering::SeqCst) >= 8 { continue; }
    ...
    pub fn workspace(&self, base: &Path, devin: bool) -> Result<AgentWorkspace, String> {
        ...
        if devin {
            builder.create(workspace.0.join(".devin")).map_err(|error| error.to_string())?;
            let server = self.config()?;
            let env: serde_json::Map<String, Value> = server["env"].as_array().unwrap().iter()
                .map(|entry| (entry["name"].as_str().unwrap().to_string(), entry["value"].clone())).collect();
            ...
            let mut file = options.open(workspace.0.join(".devin/mcp_config.json")).map_err(|error| error.to_string())?;
    ...
    pub fn rotate_session(&self) -> Result<(), String> {
        ...
        if state.turn.is_some() { return Err("An agent turn is still running".into()); }
        state.token = nonce()?;
        state.canvas_tool_calls.clear();
        Ok(())
    }

    pub fn begin(&self, turn_id: String, canvas_id: Option<String>, session_id: String) -> Result<(), String> {
        let mut state = self.state.lock().unwrap();
        if state.turn.is_some() { return Err("An agent turn is already running".into()); }
        state.canvas_tool_calls.clear();
        state.turn = Some(Turn { id: turn_id, canvas_id, session_id });
        Ok(())
    }

    pub fn finish(&self, turn_id: &str) -> bool {
        let mut state = self.state.lock().unwrap();
        if !state.turn.as_ref().is_some_and(|turn| turn.id == turn_id) { return false; }
        state.turn = None;
        state.canvas_tool_calls.clear();
        for (_, sender) in state.pending.drain() { let _ = sender.send(Err("Canvas turn ended".into())); }
        true
    }

    pub fn active_turn(&self, session_id: &str) -> Option<String> {
        self.state.lock().unwrap().turn.as_ref().filter(|turn| turn.session_id == session_id).map(|turn| turn.id.clone())
    }

    fn canvas_tool_name(value: &str) -> bool {
        let name = value.strip_prefix("mcp__kan-canvas__").unwrap_or(value);
        if TOOL_NAMES.contains(&name) { return true; }
        value.strip_prefix("Calling ")
    ...
    pub fn record_tool_call(&self, session_id: &str, update: &Value) {
        let Some(tool_call_id) = update.get("toolCallId").and_then(Value::as_str) else { return };
        let name = update.get("name")
            .or_else(|| update.get("toolName"))
            .or_else(|| update.get("title"))
            .and_then(Value::as_str);
        let mut state = self.state.lock().unwrap();
        if state.turn.as_ref().is_some_and(|turn| turn.session_id == session_id && name.is_some_and(Self::canvas_tool_name)) {
            state.canvas_tool_calls.insert(tool_call_id.to_string());
        }
    }

    pub fn allows(&self, session_id: Option<&str>, tool_call_id: Option<&str>, name: Option<&str>) -> bool {
    ...
    pub fn complete(&self, request_id: &str, turn_id: &str, result: ToolResult) -> Result<(), String> {
        let mut state = self.state.lock().unwrap();
        if !state.turn.as_ref().is_some_and(|turn| turn.id == turn_id) { return Err("Stale canvas turn".into()); }
        let sender = state.pending.remove(request_id).ok_or("Unknown canvas request")?;
        sender.send(result).map_err(|_| "Canvas request expired".into())
    }

    fn call(&self, token: &str, name: &str, arguments: Value) -> ToolResult {
        if !TOOL_NAMES.contains(&name) { return Err("Unknown canvas tool".into()); }
        let (sender, receiver) = channel();
        let request_id = nonce()?;
        let turn = {
            let mut state = self.state.lock().unwrap();
            if token != state.token { return Err("Canvas session expired".into()); }
            let turn = state.turn.clone().filter(|turn| turn.canvas_id.is_some()).ok_or("No active offline canvas turn")?;
            if state.pending.len() >= 8 { return Err("Too many pending canvas tools".into()); }
    ...
    fn dispatch(&self, request: &Value) -> Value {
        ...
        if token != self.state.lock().unwrap().token { return json!({"error": "Unauthorized canvas session"}); }
        let rpc = &request["rpc"];
        ...
        let result = match rpc["method"].as_str().unwrap_or("") {
            ...
            "tools/call" => {
                let result = self.call(token, rpc["params"]["name"].as_str().unwrap_or(""), rpc["params"].get("arguments").cloned().unwrap_or(json!({})));
                match result {
            ...
            _ => return json!({"jsonrpc": "2.0", "id": id, "error": {"code": -32601, "message": "Unsupported MCP method"}}),
        };
...
pub fn serve_stdio() -> Result<(), String> {
    let address: SocketAddr = std::env::var("KAN_CANVAS_BRIDGE_ADDR").map_err(|_| "Missing bridge address")?.parse().map_err(|_| "Invalid bridge address")?;
    if !address.ip().is_loopback() { return Err("Bridge must be loopback".into()); }
    ...
    while let Some(rpc) = read_message(&mut input)? {
        if rpc.get("id").is_none() { continue; }
        let mut socket = TcpStream::connect_timeout(&address, Duration::from_secs(3)).map_err(|_| "Canvas bridge unavailable")?;
        socket.set_read_timeout(Some(Duration::from_secs(25))).map_err(|_| "Bridge timeout unavailable")?;
        socket.set_write_timeout(Some(Duration::from_secs(3))).map_err(|_| "Bridge timeout unavailable")?;
        ...
        if response.get("jsonrpc").is_none() { return Err("Canvas bridge session expired".into()); }
...
mod tests {
    ...
    fn discovery_requires_a_session_secret_and_rotation_revokes_it() {
        ...
        assert_eq!(bridge.dispatch(&request)["result"]["tools"].as_array().unwrap().len(), 9);
    ...
    fn calls_and_permissions_are_fenced_by_the_active_turn() {
        let bridge = bridge();
        let token = bridge.state.lock().unwrap().token.clone();
        assert!(bridge.call(&token, "addNode", json!({})).unwrap_err().contains("No active"));
        ...
        assert!(bridge.begin("second".into(), Some("other".into()), "session".into()).is_err());
    ...
    fn devin_workspace_has_private_config_and_cleans_it_up() {
        ...
        assert_eq!(config["mcpServers"]["kan-canvas"]["args"], json!(["--canvas-mcp"]));
        assert!(config["mcpServers"]["kan-canvas"]["env"]["KAN_CANVAS_BRIDGE_TOKEN"].as_str().is_some());
        #[cfg(unix)]
        {
    ...
    fn tool_results_round_trip_and_cancelled_calls_cannot_complete() {
        ...
        let tools = TOOL_NAMES.iter().map(|name| json!({"name": name, "inputSchema": {"type": "object"}})).collect();
        let bridge = CanvasMcp::with_emitter(tools, Box::new(move |event| send.send(event).map_err(|error| error.to_string()))).unwrap();
        bridge.begin("turn".into(), Some("canvas".into()), "session".into()).unwrap();
        let token = bridge.state.lock().unwrap().token.clone();
        let request = json!({"token": token, "rpc": {"id": 7, "method": "tools/call", "params": {"name": "getCanvas", "arguments": {}}}});
        let pending = { let bridge = bridge.clone(); let request = request.clone(); std::thread::spawn(move || bridge.dispatch(&request)) };
        let event = receive.recv_timeout(Duration::from_secs(2)).unwrap();
        assert_eq!(event["canvasId"], "canvas");
        bridge.complete(event["requestId"].as_str().unwrap(), "turn", Ok(json!({"nodes": []}))).unwrap();
        assert_eq!(pending.join().unwrap()["result"]["isError"], false);
        let pending = { let bridge = bridge.clone(); std::thread::spawn(move || bridge.dispatch(&request)) };
        let event = receive.recv_timeout(Duration::from_secs(2)).unwrap();
        bridge.finish("turn");
        assert!(bridge.complete(event["requestId"].as_str().unwrap(), "turn", Ok(json!({}))).is_err());
