...
pub fn load(app: &AppHandle) -> Result<Preferences, String> {
    let path = app
        ...
        .join("agent-preferences.json");
    match std::fs::read(path) {
        Ok(bytes) => {
            serde_json::from_slice(&bytes).map_err(|_| "Could not read saved AI preferences".into())
        }
...
fn entry(app: &AppHandle, provider: Provider) -> Result<keyring::Entry, String> {
    keyring::Entry::new(
        &format!("{}.agent", app.config().identifier),
        match provider {
...
pub fn read_key(app: &AppHandle, provider: Provider) -> Result<String, String> {
    entry(app, provider)?
        .get_password()
        .map_err(|_| "Saved API key is unavailable. Unlock macOS Keychain or sign in again.".into())
}

#[cfg(target_os = "macos")]
pub fn write_key(app: &AppHandle, provider: Provider, key: &str) -> Result<(), String> {
    entry(app, provider)?.set_password(key).map_err(|_| {
        "Could not save API key in macOS Keychain. Allow Keychain access and try again.".into()
    })
}
...
#[cfg(not(target_os = "macos"))]
pub fn delete_key(_: &AppHandle, _: Provider) -> Result<(), String> {
    Ok(())
}
...
mod tests {
    ...
    fn supports_legacy_models_and_missing_capability() {
        let models = Models::parse(
            &json!({ "models": { "currentModelId": "a", "availableModels": [{ "modelId": "a", "name": "A" }] } }),
        );
    ...
    fn preferences_contain_no_credentials_and_default_to_no_reconnect() {
        let preferences: Preferences = serde_json::from_value(
            json!({"provider":"devin", "mode":"subscription", "modelId":"a"}),
        )
        .unwrap();
