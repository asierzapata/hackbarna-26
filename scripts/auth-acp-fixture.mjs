  ...
  openai
    ? {
        models: {
          ...
          availableModels: [
            { modelId: "model-a", name: "Fixture Fast" },
            { modelId: "model-b", name: "Fixture Reasoning" },
          ],
        },
      }
...
for await (const line of createInterface({ input: process.stdin })) {
  ...
  if (method === "initialize") {
    result = {
      protocolVersion: 1,
      agentInfo: { name: "kan-auth-fixture", title: `${provider} fixture` },
      agentCapabilities: {},
      authMethods: [
        { id: "api-key", name: "API key" },
        { id: openai ? "chat-gpt" : "devin-browser", name: "Subscription" },
      ],
    };
    ...
    writeFileSync(
      ...
      String(
        Number(
          existsSync(authenticationCount)
            ? readFileSync(authenticationCount, "utf8")
            : 0,
        ) + 1,
      ),
    );
    ...
    send({
      ...
      params: {
        ...
        update: {
          ...
          content: { type: "text", text: `Executed by ${model}` },
        },
      },
    });
