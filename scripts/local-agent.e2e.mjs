...
const drive = (...args) => {
  ...
  return ["reload", "shot"].includes(args[0])
    ? output.trim()
    : JSON.parse(output);
};
  ...
  while (Date.now() < deadline) {
    ...
    if (failOnStopped && timeout === 240000) {
      const stopped = evaluate(
        "(() => { const sim = document.querySelector('[data-testid=conversation-simulator]'); return sim?.dataset.playing === 'false' && Number(sim.dataset.cursor) > 0; })()",
      );
      if (stopped)
        throw new Error(
          `Conversation stopped without satisfying: ${expression}`,
        );
    }
    await pause(500);
  }
...
const catalog = async (method, input) => {
  run(
    `window.__agentTestCatalog = null; import('/src/lib/canvas-repository.ts').then(repo => repo[${JSON.stringify(method)}](${JSON.stringify(input)})).then(() => { window.__agentTestCatalog = {ok:true}; }).catch(error => { window.__agentTestCatalog = {error:String(error)}; }); return true;`,
  );
  await waitFor("window.__agentTestCatalog !== null");
  assert.equal(
    evaluate("window.__agentTestCatalog.ok"),
    true,
    evaluate("window.__agentTestCatalog.error"),
  );
};

try {
  assert.equal(evaluate("!!window.__TAURI_INTERNALS__"), true);
  await catalog("createOfflineCanvas", {
    id: canvasId,
    name: "Devin conversation verification",
  });
  run(`location.assign(${JSON.stringify(path)}); return true;`);
  await waitFor(
    "!!window.__kan && !!document.querySelector('[data-testid=conversation-simulator]')",
  );
  assert.equal(evaluate("window.__kan.tools.getCanvas().nodes.length"), 0);

  if (
    !evaluate(
      "document.querySelector('[data-testid=agent-status]')?.textContent.includes('Devin')",
    )
  ) {
    assert.equal(
      evaluate("!!document.querySelector('[data-testid=agent-status]')"),
      false,
      "Another provider is connected; select Devin first",
    );
    drive("click", "[data-testid=sign-in-button]");
    drive("click", "[data-testid=sign-in-devin]");
    await waitFor(
      "!!document.querySelector('[data-testid=sign-in-devin-subscription]')",
    );
    drive("click", "[data-testid=sign-in-devin-subscription]");
    await waitFor(
      "!!document.querySelector('[data-testid=agent-status]') || !!document.querySelector('[data-testid=agent-error]')",
      120000,
    );
    assert.equal(
      evaluate(
        "document.querySelector('[data-testid=agent-error]')?.textContent ?? ''",
      ),
      "",
    );
  }
  ...
  await waitFor(
    "window.__kan.tools.getCanvas().nodes.some(node => node.type === 'calendar')",
    240000,
  );
  console.log("PASS Devin created a calendar from the simulated transcript");
  await waitFor(
    "window.__kan.tools.getCanvas().nodes.some(node => node.type === 'map')",
    240000,
  );
  console.log("PASS Devin created a map from the next conversation trigger");
  await waitFor(
    "(() => { const sim = document.querySelector('[data-testid=conversation-simulator]'); return Number(sim?.dataset.cursor ?? 0) >= Number(sim?.dataset.total ?? 1) || !!sim?.querySelector('[role=alert]'); })()",
    240000,
    false,
  );
  assert.equal(
    evaluate(
      "document.querySelector('[data-testid=conversation-simulator] [role=alert]')?.textContent ?? ''",
    ),
    "",
  );

  const shapes = evaluate(
    "window.__kan.tools.getCanvas({scope:'full'}).shapes",
  );
  ...
  assert.deepEqual(calendar.props.events.map(({ start }) => start).sort(), [
    "2026-09-19",
    "2026-09-20",
  ]);
  assert.equal(map.props.markers.length, 2);
  for (const expected of [
    { label: /glovo/i, lat: 41.403, lng: 2.194 },
    { label: /norrsken/i, lat: 41.375, lng: 2.189 },
  ]) {
    const marker = map.props.markers.find(({ label }) =>
  ...
  for (const shape of [calendar, map, table])
    assert.equal(shape.meta.provenance.agentId, "assistant");
  ...
  console.log(
    JSON.stringify(
      {
        canvasId,
        nodes: shapes.map(({ id, type, title }) => ({ id, type, title })),
      },
      null,
      2,
    ),
  ...
  if (!keep && evaluate("location.pathname") === path) {
    run(
      "const stop = document.querySelector('[aria-label=\"Stop simulated conversation\"]'); if (stop) stop.click(); const signOut = document.querySelector('[data-testid=agent-sign-out]'); if (signOut) signOut.click(); return true;",
    );
    await waitFor("!document.querySelector('[data-testid=agent-sign-out]')");
    const ids = evaluate(
      "window.__kan.editor.getCurrentPageShapes().map(shape => shape.id)",
    );
    if (ids.length)
      evaluate(
        `window.__kan.tools.removeNodes(${JSON.stringify({ shapeIds: ids })})`,
      );
    await pause(1500);
